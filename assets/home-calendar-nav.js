...
(function(){'use strict';
  function boot(){
    ...
    function openDetails(day){
      ...
      var tithi=day.tithi||{},nak=day.nakshatra||{},yoga=day.yoga||{},karana=day.karana||{},sun=day.sun||{},moon=day.moon||{};
      var rahu=day.rahuKaal?clean(day.rahuKaal.start)+' – '+clean(day.rahuKaal.end):'';
      var gulika=day.gulikaKaal?clean(day.gulikaKaal.start)+' – '+clean(day.gulikaKaal.end):'';
      var rows=detailValue('तिथि',tithi.name,'☀️')+detailValue('पक्ष',tithi.paksha,'🌙')+detailValue('तिथि समाप्ति',tithi.end,'⏳')+detailValue('नक्षत्र',nak.name,'⭐')+detailValue('योग',yoga.name,'🕉️')+detailValue('करण',karana.name,'◐')+detailValue('राशि',day.rashi,'♈')+detailValue('सूर्योदय',sun.sunrise,'🌅')+detailValue('सूर्यास्त',sun.sunset,'🌇')+detailValue('चन्द्रोदय',moon.rise,'🌙')+detailValue('चन्द्रास्त',moon.set,'🌘')+detailValue('राहुकाल',rahu,'⚠️')+detailValue('गुलिककाल',gulika,'◒')+detailValue('नेपाल संवत्',day.nepalSambat,'🇳🇵');
      modal.innerHTML='<div class="np-date-modal-backdrop" data-close-date></div><div class="np-date-modal-card"><button type="button" class="np-date-modal-close" aria-label="बन्द गर्नुहोस्" data-close-date>×</button><div class="np-date-modal-top"><span class="np-date-modal-badge">'+(holiday?'🇳🇵 बिदा':'📅 दिनको विवरण')+'</span><h3 id="npDateModalTitle">'+esc(day.bs.display)+'</h3><p>'+esc(day.weekday&&day.weekday.nepali)+' · '+esc(formatAd(day.ad.date))+'</p></div><div class="np-date-detail-grid">'+rows+'</div>'+(eventText?'<div class="np-date-events"><small>पर्व / कार्यक्रम</small><strong>'+(holiday?'🇳🇵 ':'')+esc(eventText)+'</strong></div>':'')+'<div class="np-date-modal-actions"><button type="button" class="np-date-open-calendar" data-open-calendar>पूर्ण पात्रोमा यो मिति खोल्नुहोस् <span>→</span></button></div></div>';
