...
// Keep the homepage date locked to Nepal time, regardless of the visitor's timezone.
...
function render(data,x){
 ...
 if(cal){const ds=(data.days||[]).filter(d=>Number(d?.bs?.month)===Number(x.bs.month)).sort((a,b)=>Number(a.bs.day)-Number(b.bs.day));
 const first=ds.find(d=>Number(d.bs.day)===1)||ds[0];
 let h='<div class="np-home-cal-head"><strong>'+esc(MONTHS[Number(x.bs.month)-1]||x.bs.monthNepali||'पात्रो')+' '+nep(x.bs.year)+'</strong><small>आज: '+nep(x.bs.day)+'</small></div><div class="np-home-cal-grid">'+WEEK.map(w=>'<div class="np-home-cal-week">'+w+'</div>').join('');
 if(!ds.length){cal.innerHTML='<div class="np-runtime-error">आजको पात्रो उपलब्ध छैन। <button id="npRetryCalendar">पुनः प्रयास</button></div>';document.getElementById('npRetryCalendar')?.addEventListener('click',boot,{once:true});}
 else {for(let i=0;i<Number(first?.weekday?.index||0);i++)h+='<div></div>';
 for(const d of ds){const isToday=d.ad?.date===x.ad?.date;h+='<a class="np-home-cal-cell '+(isToday?'is-today':'')+'" '+(isToday?'aria-current="date"':'')+' href="'+ROOT+'calendar/?date='+encodeURIComponent(d.ad.date)+'"><b>'+nep(d.bs.day)+'</b><small>'+esc(d.ad?.day||'')+'</small>'+(d.festival?'<span>'+esc(d.festival)+'</span>':'')+'</a>'}
 cal.innerHTML=h+'</div>'}
