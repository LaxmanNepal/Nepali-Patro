...
function render(x){
 ...
 if(p){const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));const rows=[['☀️','तिथि',x.tithi?.name],['🌙','पक्ष',x.tithi?.paksha],['⭐','नक्षत्र',x.nakshatra?.name],['🕉️','योग',x.yoga?.name]];p.innerHTML=rows.map(([icon,a,b])=>'<div class="np-stat"><i class="np-panchanga-icon">'+icon+'</i><span><small>'+esc(a)+'</small><b>'+esc(b||'—')+'</b></span></div>').join('')}
}
...
async function loadScript(src){if(document.querySelector(`script[data-np-src="${src}"]`))return;await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.dataset.npSrc=src;s.onload=resolve;s.onerror=()=>reject(Error('Failed to load '+src));document.head.appendChild(s)})}
async function ensureMonthCalendar(){
 try{
  ...
  await loadScript('assets/home-calendar-modal-enhancement.js?v=20260917-02');
  const css=document.createElement('link');css.rel='stylesheet';css.href='css/home-calendar-modal-enhancement.css?v=20260917-01';document.head.appendChild(css);
