...
const addNepalInterestLink=()=>{
  const desktop=document.querySelector('.np-header nav');
  if(desktop&&!desktop.querySelector('[data-nepal-interest]')){
    const a=document.createElement('a');
    a.href=INTEREST_URL;a.textContent='नेपाल';a.title='नेपाल — बैंक ब्याजदर';a.dataset.nepalInterest='true';
    desktop.appendChild(a);
  }
  const mobile=document.getElementById('mobileMenu');
  if(mobile&&!mobile.querySelector('[data-nepal-interest]')){
    const a=document.createElement('a');
    a.href=INTEREST_URL;a.textContent='🇳🇵 नेपाल — बैंक ब्याजदर';a.dataset.nepalInterest='true';
