...
FEEDS = {
    ...
    'sports': [('Hamro Khelkud','https://www.hamrokhelkud.com/feed'),('Goal Nepal','https://www.goalnepal.com/rss'),('Khelpati','https://www.khelpati.com/feed'),('Nepal Sportz','https://nepalsportz.com/feed/'),('CricNepal','https://www.cricnepal.com/feed'),('Onlinekhabar Sports','https://www.onlinekhabar.com/content/sports/feed'),('News of Nepal Sports','https://www.newsofnepal.com/category/sports/feed/'),('Cricket Nepal','https://cricketnepal.org.np/feed/')],
...
def classify(category,title):
    if category=='national' and POLITICS.search(title):return 'politics'
    return category
...
def parse_feed(source,url,category):
    ...
    for item in [e for e in root.iter() if e.tag.split('}')[-1] in ('item','entry')][:25]:
        ...
        # The requested news experience is Nepali-only. Reject titles that are not meaningfully Devanagari.
