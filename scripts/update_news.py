...
FEEDS = {
    ...
    'sports': [
        ('Hamro Khelkud','https://www.hamrokhelkud.com/feed'),('Goal Nepal','https://www.goalnepal.com/rss'),('Khelpati','https://www.khelpati.com/feed'),('Nepal Sportz','https://nepalsportz.com/feed/'),('CricNepal','https://www.cricnepal.com/feed'),('Onlinekhabar Sports','https://www.onlinekhabar.com/content/sports/feed'),('News of Nepal Sports','https://www.newsofnepal.com/category/sports/feed/'),('Cricket Nepal','https://cricketnepal.org.np/feed/')
    ],
    'business': [
...
def parse_feed(source,url,category):
    ...
    nodes=[]
    for e in root.iter():
        if e.tag.split('}')[-1] in ('item','entry'):nodes.append(e)
    for item in nodes[:25]:
        ...
        summary=first(item,['description','summary','content','encoded'])
        ...
        # Keep the site's Nepali feed content. Reject articles whose title is overwhelmingly non-Devanagari.
        if len(DEV.findall(title)) < 3:continue
        out.append({'title':title,'summary':summary[:300],'link':link,'source':source,'category':category,'published':parse_date(published)})
    return out
