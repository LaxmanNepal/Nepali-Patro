...
blocks. It intentionally requires both a rate and nearby product context.
"""
...
RATE_CONTEXT_RE = re.compile(
    r"\b(saving|deposit|fixed|fd|recurring|call|loan|advance|base\s+rate"
            ...
            for column_index, cell in enumerate(cells):
                ...
                label = next(
                    (
                        value
                        for value in cells
                        if value != cell and RATE_CONTEXT_RE.search(value)
                    ),
                    cells[0],
                )
                records.append(
...
def parse_html_blocks(html: str) -> list[dict]:
    """Extract conservative rate records from non-table page structures."""
    soup = BeautifulSoup(html, "html.parser")
    ...
    candidates = soup.find_all(
        ["article", "section", "li", "div", "p", "dd", "dt"]
    )
    ...
    for node in candidates:
        ...
        rates = RATE_RE.findall(text.replace(",", ""))
        if not rates or not RATE_CONTEXT_RE.search(text):
            continue

        label_node = node.find_previous(
            ["h1", "h2", "h3", "h4", "h5", "strong", "b"]
        )
        label = (
            " ".join(label_node.stripped_strings)
            if label_node
            else text[:180]
        )
...
def validate_rates(records: list[dict]) -> list[dict]:
    return [
        ...
        and 0 <= float(record["rate"]) <= 100
        and str(record.get("raw", "")).strip()
    ]
