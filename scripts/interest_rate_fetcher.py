...
EFFECTIVE_RE = re.compile(
    r"(?:effective(?:\s+from)?|effective date|लागु|प्रभावकारी).{0,160}",
    re.I | re.S,
)
    ...
    if prior and prior.get("status") == "verified" and prior.get("rates"):
        return {
            **prior,
            ...
            "fetchStatus": failed_item.get("status"),
            "fetchError": failed_item.get("error"),
        }
...
def main() -> int:
    ...
    payload = {
        ...
        "publicationRule": (
            "Only explicitly sourced and semantically recognized records "
            "are eligible for comparison."
        ),
        "health": {
            "totalBanks": len(results),
            "verifiedBanks": verified,
            "hasUsableRates": verified > 0,
        },
        "banks": results,
    }
    ...
    HISTORY.write_text(
        json.dumps(previous_history, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
