...
const css='/Nepali-Patro/css/shared-shell.css';
const js='/Nepali-Patro/js/shared-shell.js';
const sw='/Nepali-Patro/sw.js';
...
const registration=`<script>if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('${sw}',{scope:'/Nepali-Patro/'}).catch(()=>{}));}</script>`;
for(const file of files(root)){
 ...
 s=s.replace(/<script[^>]+serviceWorker\.register[^>]*><\/script>\s*/gi,'');
 ...
 s=s.replace(/<body([^>]*)>/i,`<body$1>${header}`);
 s=s.replace(/<\/body>/i,`${footer}<script src="${js}?v=${version}" data-shared-build="${version}"></script>${registration}</body>`);
 fs.writeFileSync(file,s);
 console.log(`shared shell inlined: ${path.relative(root,file)} (${version})`);
}
