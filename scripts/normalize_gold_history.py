...
def normalize_record(item, source='NEGOSIDA'):
    record = {
    ...
    if item.get('provenance'):
        record['provenance'] = item['provenance']
    return record
...
def main():
    ...
    # Preserve previously recovered historical snapshots; the live feed can be sparse.
    for item in existing.get('records', []):
        if item.get('date_ad'):
            rows[item['date_ad']] = item

    # Live official feed wins over archived snapshots for the same date.
    ...
    history = [rows[k] for k in sorted(rows)][-365:]
    payload = {
        'schema': SCHEMA,
    ...
    print(f'Normalized gold history: {len(history)} records')
