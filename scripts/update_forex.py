...
def find_bs(ad_date):
    # Use the repository's generated calendar datasets, so the published feed stays local after build.
        ...
        try:
            ...
            for d in data.get('days',[]):
                if d.get('ad',{}).get('date')==ad_date:
                    return d.get('bs',{}).get('display') or f"{d.get('bs',{}).get('monthName','')} {d.get('bs',{}).get('day','')}, {y}"
        except Exception: pass
    return ''

def main():
    today=date.today().isoformat()
    qs=urllib.parse.urlencode({'page':1,'per_page':100,'from':today,'to':today})
    payload=get(ROOT+'?'+qs)
    rows=(payload.get('data') or {}).get('payload') or []
    if not rows: raise RuntimeError('NRB returned no rates for '+today)
    item=rows[0]; rates=[]
    ...
    out={'date_bs':find_bs(item.get('date',today)),'date_ad':item.get('date',today),'base':'NPR','rates':rates}
