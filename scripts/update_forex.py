...
ROOT='https://www.nrb.org.np/api/forex/v1/rates'
OUT='feeds/forex.json'
FLAGS={'USD':'🇺🇸','EUR':'🇪🇺','GBP':'🇬🇧','AUD':'🇦🇺','CAD':'🇨🇦','SGD':'🇸🇬','JPY':'🇯🇵','CNY':'🇨🇳','SAR':'🇸🇦','QAR':'🇶🇦','AED':'🇦🇪','MYR':'🇲🇾','KRW':'🇰🇷','SEK':'🇸🇪','DKK':'🇩🇰','HKD':'🇭🇰','KWD':'🇰🇼','BHD':'🇧🇭','OMR':'🇴🇲','THB':'🇹🇭','INR':'🇮🇳','CHF':'🇨🇭'}

def get(url):
    req=urllib.request.Request(url,headers={'User-Agent':'Nepali-Patro/2.0'})
...
def main():
    ...
    for r in item.get('rates') or []:
        ...
        try:
            unit=float(c.get('unit',1) or 1); buy=float(r.get('buy')); sell=float(r.get('sell'))
        except (TypeError,ValueError): continue
        rates.append({'currency':code,'name':c.get('name') or code,'flag':FLAGS.get(code,'🌐'),'unit':int(unit) if unit.is_integer() else unit,'buy':buy,'sell':sell})
    if len(rates)<5: raise RuntimeError(f'NRB returned only {len(rates)} valid currencies')
    rates.sort(key=lambda x:x['currency'])
    ad=item.get('date',today.isoformat())
    out={'date_bs':find_bs(ad),'date_ad':ad,'base':'NPR','source':'Nepal Rastra Bank','sourceUrl':'https://www.nrb.org.np/forex/','updatedAt':datetime.now(timezone.utc).isoformat(),'rates':rates}
