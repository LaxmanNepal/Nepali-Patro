...
async function initEngine() {
  ...
  ready = (async () => {
    ...
    // The package requires WASM initialization before ANY method call.
    await instance.init();
    swe = instance;
    // The previous implementation requested CalculationFlag.SwissEphemeris
    // without loading the .se1 files. That is the direct cause of the
    // "SwissEphemeris not initialized" runtime error. Load them once.
    try {
      await instance.loadStandardEphemeris();
      usingSwissFiles = true;
    } catch (e) {
      // Keep the application usable offline: Moshier is built into the WASM.
      usingSwissFiles = false;
    }
    $('engineState').textContent = '✓ गणना इन्जिन तयार';
    $('engineMeta').textContent = usingSwissFiles ? 'Swiss Ephemeris · Standard ephemeris loaded' : 'Swiss Ephemeris WASM · Moshier fallback';
    return instance;
  })().catch(err => {
    ready = null;
    swe = null;
