...
/* Browser UI wrapper for the original Shuvayatra/preeti converter.
 * Original source/license and attribution are shown on the page.
 */
window.PreetiUnicode = { convert: preeti };
...
function updateCounts(){
  inputCount.textContent = input.value.length.toLocaleString("ne-NP") + " अक्षर";
  outputCount.textContent = output.value.length.toLocaleString("ne-NP") + " अक्षर";
}
function convertNow(){
  output.value = preeti(input.value, "Preeti");
  updateCounts();
  status.textContent = input.value ? "रूपान्तरण सम्पन्न" : "तयार छ";
}
function flash(message){
  status.textContent = message;
  setTimeout(()=>{ if(status.textContent===message) status.textContent="तयार छ"; },1800);
}
input.addEventListener("input", convertNow);
document.getElementById("convertBtn").addEventListener("click", convertNow);
document.getElementById("clearBtn").addEventListener("click", ()=>{
  input.value=""; output.value=""; updateCounts(); flash("Text clear गरियो"); input.focus();
});
document.getElementById("copyBtn").addEventListener("click", async ()=>{
  if(!output.value){ flash("पहिले text रूपान्तरण गर्नुहोस्"); return; }
  try { await navigator.clipboard.writeText(output.value); }
  catch { output.select(); document.execCommand("copy"); }
  flash("Unicode text copied");
});
document.getElementById("pasteBtn").addEventListener("click", async ()=>{
  try { input.value=await navigator.clipboard.readText(); convertNow(); input.focus(); flash("Clipboard बाट paste गरियो"); }
  catch { flash("Clipboard permission उपलब्ध भएन"); }
});
document.getElementById("downloadBtn").addEventListener("click", ()=>{
  if(!output.value){ flash("डाउनलोड गर्न पहिले convert गर्नुहोस्"); return; }
  const blob=new Blob([output.value],{type:"text/plain;charset=utf-8"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a"); a.href=url; a.download="preeti-to-unicode.txt"; a.click();
  URL.revokeObjectURL(url); flash("TXT file तयार भयो");
});
document.getElementById("swapBtn").addEventListener("click", ()=>{
  const examples=['sf7df08"','g]kfn','g]kfnL'];
  input.value=examples.find(x=>x!==input.value)||examples[0];
  convertNow(); input.focus();
});
document.querySelectorAll(".pu-example").forEach(btn=>btn.addEventListener("click",()=>{
  input.value=btn.dataset.example||""; convertNow(); input.focus();
}));
document.addEventListener("keydown",e=>{
  if((e.ctrlKey||e.metaKey)&&e.key==="Enter"){e.preventDefault();convertNow();}
  if((e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key.toLowerCase()==="c"){e.preventDefault();document.getElementById("copyBtn").click();}
});
updateCounts();
